...
const batch_1 = require("../anchor/batch");
const config_1 = require("./config");
const http_1 = require("./http");
const hash_1 = require("../hash");
const errors_1 = require("../errors");
const cache_1 = require("../verify/cache");
/**
...
class SipHeron {
    constructor(config) {
        this.anchors = {
            revoke: async (anchorId, options) => {
                if (!this.config.apiKey) {
                    throw new errors_1.AuthenticationError('apiKey is required to revoke anchors.');
                }
                await (0, http_1.withRetry)(() => this.http.post(`/api/anchors/${anchorId}/revoke`, options), this.config.retries);
            },
            getVersionChain: async (anchorId) => {
                ...
                return response.data.chain.map((c) => this._mapAnchorResponse(c));
            }
        };
        this.config = (0, config_1.resolveConfig)(config);
        this.http = (0, http_1.createHttpClient)(this.config);
        if (this.config.cache) {
            this.verifyCache = new cache_1.VerificationCache(this.config.cache);
        }
    }
    /**
    ...
    async anchor(options) {
        ...
        if (options.file) {
            hash = await (0, hash_1.hashDocument)(options.file, { algorithm });
        }
        else {
            hash = (0, hash_1.normalizeHash)(options.hash);
            if (!(0, hash_1.isValidHash)(hash, algorithm)) {
                const info = (0, hash_1.getAlgorithmInfo)(algorithm);
                throw new errors_1.ValidationError(`Invalid hash format for ${info.label}. Must be a ${info.outputLength}-character lowercase hex string.`);
            }
        }
    ...
    async anchorBatch(options) {
        ...
        return (0, batch_1.anchorBatch)(this, options);
    }
    /**
    ...
    async verify(options) {
        ...
        if (options.file) {
            ...
            if (!(0, hash_1.isValidHash)(hash, algorithm)) {
                const info = (0, hash_1.getAlgorithmInfo)(algorithm);
                throw new errors_1.ValidationError(`Invalid hash format for ${info.label}. Must be a ${info.outputLength}-character hex string.`);
            }
        }
        ...
        const response = await (0, http_1.withRetry)(() => this.http.post(endpoint, { hash, hashAlgorithm: algorithm }), this.config.retries);
     ...
     */
    async getStatus(hashOrId) {
        let normalized = hashOrId.toLowerCase();
        const isId = normalized.includes('-') || normalized.startsWith('anc_');
        if (!isId && !(0, hash_1.isValidHash)(normalized)) {
            throw new errors_1.ValidationError('Invalid reference format. Must be a SHA-256 hash or an Anchor ID.');
        }
        if (isId && this.config.apiKey) {
            ...
            const match = chain.find((c) => c.id === normalized) || chain[0];
            if (match && match.hash) {
    ...
    _mapAnchorResponse(data) {
        ...
        return {
            id: String(anchor.id || ''),
            hash,
            hashAlgorithm: anchor.hashAlgorithm || 'sha256',
