...
exports.verifyHashStandalone = verifyHashStandalone;
/**
 ...
 */
async function verifyLocally(fileOrHash, anchorHash) {
    if (!anchorHash || !(0, hash_1.isValidHash)((0, hash_1.normalizeHash)(anchorHash))) {
        throw new errors_1.ValidationError('anchorHash must be a valid 64-character SHA-256 hex string');
    }
    let computedHash;
    if (Buffer.isBuffer(fileOrHash)) {
        computedHash = await (0, hash_1.hashDocument)(fileOrHash);
    }
    else {
        const normalized = (0, hash_1.normalizeHash)(fileOrHash);
        if (!(0, hash_1.isValidHash)(normalized)) {
            throw new errors_1.ValidationError('Invalid hash format. Must be 64 hex characters.');
        }
        computedHash = normalized;
    }
    return {
        authentic: (0, compare_1.compareHashes)(computedHash, anchorHash),
        computedHash,
        anchorHash: (0, hash_1.normalizeHash)(anchorHash),
    };
}
/**
 ...
 */
async function verifyHashStandalone(hash, apiBaseUrl = 'https://api.sipheron.com') {
    const normalized = (0, hash_1.normalizeHash)(hash);
    if (!(0, hash_1.isValidHash)(normalized)) {
        throw new errors_1.ValidationError('Invalid hash format. Must be a 64-character SHA-256 hex string.');
    }
